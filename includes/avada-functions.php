...
if( ! function_exists( 'avada_render_related_posts' ) ) {
	...
	function avada_render_related_posts( $post_type = 'post' ) {
		...
		if ( $post_type == 'post' ) {
			...
			$main_heading =  __( 'This might interest you as well', 'stylecurve' );
		...
		if ( fusion_get_option( $theme_option_name, 'related_posts', get_the_ID() ) == 'yes' ||
			...
			if ( $related_posts->have_posts() ) {
				$html .= '<div class="related-posts single-related-posts">';
					...
					$html .= sprintf( '<div class="fusion-carousel" data-imagesize="%s" data-metacontent="%s" data-autoplay="%s" data-touchscroll="%s" data-columns="%s" data-itemmargin="%s" data-itemwidth="300" data-touchscroll="yes" data-scrollitems="%s">', 
